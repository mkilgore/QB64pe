...
int test_single_rw() {
    ...
    ret += test_assert_ints(0, libqb_buffer_length(&buffer));

    libqb_buffer_write(&buffer, str, sizeof(str));

    ret += test_assert_ints(sizeof(str), libqb_buffer_length(&buffer));

    char read_buf[sizeof(str)];
    size_t read_len = libqb_buffer_read(&buffer, read_buf, sizeof(str));

    ret += test_assert_ints(sizeof(str), read_len);
    ret += test_assert_buffers(str, read_buf, sizeof(str));
    ret += test_assert_ints(0, libqb_buffer_length(&buffer));
...
int test_multiple_rw() {
    ...
    for (int i = 0; i < count; i++) {
        ...
        length += sizeof(str);
        ret += test_assert_ints_with_name(id, length, libqb_buffer_length(&buffer));
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        ret += test_assert_ints_with_name(id, sizeof(str), read_len);
        ret += test_assert_buffers_with_name(id, str, read_buf, sizeof(str));
        ret += test_assert_ints_with_name(id, length, libqb_buffer_length(&buffer));
    }
...
int test_partial_read() {
    ...
    for (int i = 0; i < length / str_len; i++) {
        ...
        ret += test_assert_ints_with_name(id, str_len, read_len);

        // Check the prefix, without the number
        ret += test_assert_buffers_with_name(id, "FOOBAR", read_buf, str_len - 1);

        // Check the number at the end
        ret += test_assert_ints_with_name(id, '1' + i, read_buf[6]);

        ret += test_assert_ints_with_name(id, length_left, libqb_buffer_length(&buffer));
    }
...
int test_full_read() {
    ...
    ret += test_assert_ints(sizeof(full_buf), read_len);
    ret += test_assert_buffers("FOOBAR" "FOOBAR" "FOOBAR" "FOOBAR" "FOOBAR" "FOOBAR", full_buf, sizeof(full_buf));

    libqb_buffer_clear(&buffer);
    return ret;
}

int test_read_past_end() {
    int ret = 0;
    struct libqb_buffer buffer;
    ...
    ret += test_assert_ints(str_len, libqb_buffer_length(&buffer));

    char read_buf[200];
    size_t read_len = libqb_buffer_read(&buffer, read_buf, sizeof(read_buf));

    ret += test_assert_ints(str_len, read_len);
    ret += test_assert_buffers(str, read_buf, str_len);
    ret += test_assert_ints(0, libqb_buffer_length(&buffer));

    read_len = libqb_buffer_read(&buffer, read_buf, sizeof(read_buf));

    ret += test_assert_ints(0, read_len);
    ret += test_assert_ints(0, libqb_buffer_length(&buffer));
...
int test_read_write_multiple_partial() {
    int ret = 0;
    struct libqb_buffer buffer;
    ...
    for (int i = 0; i < sizeof(strs) / sizeof(*strs); i++) {
        ...
        length += str_len;
        ret += test_assert_ints(length, libqb_buffer_length(&buffer));
    }
    ...
    ret += test_assert_ints(12, read_len);
    ret += test_assert_ints(length, libqb_buffer_length(&buffer));
    ret += test_assert_buffers("FOOBAR1FOOBA", temp_buf, 12);

    read_len = libqb_buffer_read(&buffer, temp_buf, 20);
    length -= 20;

    ret += test_assert_ints(20, read_len);
    ret += test_assert_ints(length, libqb_buffer_length(&buffer));
    ret += test_assert_buffers("R2FOOBAR3FOOBAR4FOOB", temp_buf, 20);

    read_len = libqb_buffer_read(&buffer, temp_buf, 25);
    length -= 25;

    ret += test_assert_ints(25, read_len);
    ret += test_assert_ints(length, libqb_buffer_length(&buffer));
    ret += test_assert_buffers("AR5FOOBAR6FOOBAR7FOOBAR8F", temp_buf, 25);

    // Read past the end, should only read the 6 characters left
    read_len = libqb_buffer_read(&buffer, temp_buf, 20);
    length -= 6;

    ret += test_assert_ints(6, read_len);
    ret += test_assert_ints(length, libqb_buffer_length(&buffer));
    ret += test_assert_buffers("OOBAR9", temp_buf, 6);
...
int test_read_write_interweaved() {
    int ret = 0;
    struct libqb_buffer buffer;
    ...
    ret += test_assert_ints(9, read_len);
    ret += test_assert_ints(length, libqb_buffer_length(&buffer));
    ret += test_assert_buffers("R2FOOBAR3", temp_buf, 9);
    ...
    ret += test_assert_ints(14, read_len);
    ret += test_assert_ints(length, libqb_buffer_length(&buffer));
    ret += test_assert_buffers("FOOBAR4FOOBAR5", temp_buf, 14);
...
int main() {
    ...
    ret = run_tests("buffer", tests, sizeof(tests) / sizeof(*tests));
