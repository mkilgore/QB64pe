...
static void libqb_vlog(loglevel lvl, logscope scope, const char *file, const char *func, int line, const char *fmt, va_list args)
{
    ...
    if (lvl == loglevel::Error)
        entry.stacktrace = libqb_log_get_stacktrace();
...
void libqb_log(loglevel lvl, logscope scope, const char *file, const char *func, int line, const char *fmt, ...) {
    ...
    va_list args;
    va_start(args, fmt);
    libqb_vlog(lvl, scope, file, func, line, fmt, args);
    va_end(args);
}

void libqb_log_qbs(loglevel lvl, logscope scope, const char *file, const char *func, int line, qbs *str) {
    libqb_log(lvl, scope, file, func, line, "%.*s", (int)str->len, (const char *)str->chr);
}
